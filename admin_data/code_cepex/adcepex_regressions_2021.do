...
program rct_regression_business
	...
	* ITT: ancova plus stratification dummies						
			* ITT: ancova plus stratification dummies
			eststo `var'1: reg `var' i.treatment c.`var'l? i.miss_`var'_l? i.strata if annee == 2021, cluster(id)
			estadd local lags "Yes"
			estadd local strata "Yes"

			* ATT, IV		
			eststo `var'2: ivreg2 `var' c.`var'l? i.miss_`var'_l? i.strata (take_up = i.treatment) if annee == 2021, cluster(id) first
			estadd local lags "Yes"
			estadd local strata "Yes"
...
rct_regression_business ca_local_dt ca_ttc_dt resultatall_dt profitable resultatall_dt_pct, gen(business)
exit 	
***********************************************************************
...
foreach outcome in `varlist' {
	...
	* ITT: ancova plus stratification dummies						
					* ITT: ancova plus stratification dummies
					eststo `outcome'_`group'1: reg `outcome' i.treatment c.`outcome'l? i.miss_`outcome'_l? i.strata if `cond' &  annee == 2021, cluster(id)
					estadd local lags "Yes"
					estadd local strata "Yes"
					...
					* ATT, IV		
					eststo `outcome'_`group'2: ivreg2 `outcome' c.`outcome'l? i.miss_`outcome'_l? i.strata (take_up = i.treatment) if `cond' & annee == 2021, cluster(id) first
					estadd local lags "Yes"
					estadd local strata "Yes"
