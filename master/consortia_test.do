...
replace check_again =3 if exprep_inv<0 & ​​exprep_inv >900000
...
replace check_again = 2 if Profit>ca & ca!=. & Profit!=. 
...
replace check_again 2 if eregexm comptable_email, regex("[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}") == 0
...
replace check_again 2 if eregexm `var', regex("^\+216[0-9]{8}$") == 0
...
replace needs_check = 2 if `var'>200 & `var'<0 & surveyround==2
...
replace check_again=2 if ca_exp>0 & ca_exp!=.  & exp_pays==. 
...
replace check_again=2 if ca_exp>0 & ca_exp!=. & exp_pays==0 
...
replace check_again=2 if ((ca_exp2020>0 & ca_exp2020!=.) |(ca_exp2019>0 & ca_exp2019!=.)|(ca_exp2018>0 & ca_exp2018!=.)|(ca_exp2021>0 & ca_exp2021!=.)) & ca_exp==0 
...
export excel id_plateforme miss validation check_again nombre_questions comptable_numero comptable_email Numero1 Numero2 questions commentaires_ElAmouri ///
commentsmsb correction_propose correction_valide ca ca_exp Profit ca_2021 ca_exp_2021 using "fiche_de_correction.xlsx" if check_again>=1, firstrow(variables) replace
restore
