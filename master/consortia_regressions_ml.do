***********************************************************************
* 			Master midline analysis/regressions				  
***********************************************************************
*																	  
...
local business_vars "age ihs_ca_w99_k4 ihs_costs_w99_k4 ihs_profit_w99_k1 ihs_employes_w99_k1"
...
iebaltab ca ca_exp profit capital employes fte_femmes age exp_pays exp_inv exprep_couts inno_rd net_nb_dehors net_nb_fam net_nb_qualite mpi eri if surveyround == 1 & refus == 0, grpvar(treatment) ftest fmissok  vce(robust) format(%12.2fc) save(baltab_midline_all) replace
						 
	* w/o outliers
iebaltab ca ca_exp profit capital employes fte_femmes age exp_pays exp_inv exprep_couts inno_rd net_nb_dehors net_nb_fam net_nb_qualite mpi eri if surveyround == 1 & refus == 0 & id_plateforme != 1092, grpvar(treatment) ftest fmissok vce(robust) format(%12.2fc) save(baltab_midline_wo_outlier) replace 
	...
	* Put all regressions into one table
		* Top panel: ATE
		...
		esttab `regressions' using "rt_`generate'.tex", replace ///
				...
				collabels(none) ///	do not use statistics names below models
				label 		/// specifies EVs have label
				drop(_cons *.strata_final *y0 ?.missing_bl_*) /// L.*  oL.* 
				noobs
...
program rct_regression_empowerment
	...
	syntax varlist(min=1 numeric), GENerate(string)
		foreach var in `varlist' {		// do following for all variables in varlist seperately	
			...
			* ITT: ancova plus stratification dummies
			eststo `var'1: reg `var' i.treatment l.`var' i.missing_bl_`var' i.strata_final if surveyround == 2, cluster(id_plateforme)
			estadd local bl_control "Yes"
			estadd local strata "Yes"

			* ATT, IV		
			eststo `var'2: ivreg2 `var' l.`var' i.missing_bl_`var' i.strata_final (take_up = i.treatment) if surveyround == 2, cluster(id_plateforme) first
			estadd local bl_control "Yes"
			estadd local strata "Yes"
...
coefplot ///
	...
	keep(*treatment take_up) drop(_cons) xline(0) ///
		...
		swapnames /// swaps coeff & equation names after collecting result
		levels(95) ///
		eqrename(`1'1 = `"Management practices (ITT)"' `1'2 = `"Management practices (TOT)"' `2'1 = `"Innovations (ITT)"' `2'2 = `"Innovations (TOT)"' `3'1 = `"Innovated (ITT)"' `3'2 = `"Innovated (TOT)"' `4'1 = `"Export readiness (ITT)"' `4'2 = `"Export readiness (TOT)"' `5'1 = `"Export readiness SSA, ihs (ITT)"' `5'2 = `"Export readiness SSA, ihs (TOT)"') ///
		xtitle("Treatment coefficient", size(medium)) ///  
		leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
		name(ml_`generate'_cfplot, replace)
...
rct_regression_kt mpi innovations innovated eri eri_ssa, gen(kt)

}

***********************************************************************
* 	PART 12: Midline results - regression table business performance outcomes
***********************************************************************
...
esttab e(RW) using rw_`generate'.tex, replace
		...
		* Put all regressions into one table
		...
		esttab `regressions' using "rt_`generate'.tex", append ///
				...
				posthead("\hline \\ \multicolumn{7}{c}{\textbf{Panel B: Treatment Effect on the Treated (TOT)}} \\\\[-1ex]") ///
				cells(b(star fmt(3)) se(par fmt(3)) p(fmt(3)) rw ci(fmt(2))) ///
				stats(control_mean control_sd N strata bl_control, fmt(%9.2fc %9.2fc %9.0g %15s %15s)  labels("Control group mean" "Control group SD" "Observations" "Strata controls" "Y0 controls")) ///
				drop(_cons *.strata_final ?.missing_bl_* L.*) ///
				star(* 0.1 ** 0.05 *** 0.01) ///
				mlabels(none) nonumbers ///		do not use varnames as model titles
				collabels(none) ///	do not use statistics names below models
...
coefplot ///
	...
	(`2'1, pstyle(p2)) (`2'2, pstyle(p2)) ///
	(`3'1, pstyle(p3)) (`3'2, pstyle(p3)) ///
	(`5'1, pstyle(p4)) (`5'2, pstyle(p4)) ///
	(`6'1, pstyle(p5)) (`6'2, pstyle(p5)) ///
	(`7'1, pstyle(p6)) (`7'2, pstyle(p6)), ///
	keep(*treatment take_up) drop(_cons) xline(0) ///
		asequation /// name of model is used
		swapnames /// swaps coeff & equation names after collecting result
		levels(95) ///
		eqrename(`1'1 = `"Total sales (ITT)"' `1'2 = `"Total sales (TOT)"' `2'1 = `"Export (ITT)"' `2'2 = `"Export (TOT)"' `3'1 = `"Costs (ITT)"' `3'2 = `"Costs (TOT)"' `5'1 = `"Profit, pct (ITT)"' `5'2 = `"Profit, pct (TOT)"' `6'1 = `"Employees (ITT)"' `6'2 = `"Employees (TOT)"' `7'1 = `" Female Employees (ITT)"' `7'2 = `"Female employees (TOT)"') ///
		xtitle("Treatment coefficient", size(medium)) ///  
		leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
		name(ml_`generate'1_cfplot, replace)
		
gr export ml_`generate'1_cfplot.png, replace
...
coefplot ///
	...
	keep(*treatment take_up) drop(_cons) xline(0) ///
		...
		swapnames /// swaps coeff & equation names after collecting result
		levels(95) ///
		eqrename(`4'1 = `"Profit, ihs (ITT)"' `4'2 = `"Profit, ihs (TOT)"') ///
		xtitle("Treatment coefficient", size(medium)) ///  
		leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
		name(ml_`generate'2_cfplot, replace)
		
gr export ml_`generate'2_cfplot.png, replace

	
end

	* apply program to business performance outcomes
rct_regression_business ihs_ca_w99_k4 ihs_ca_exp_w99_k4 ihs_costs_w99_k4 ihs_profit_w99_k1 profit_pct ihs_employes_w99_k3 car_empl1_w99_k3, gen(business_outcomes)

}	

* new adjusting all to k = 1
...
coefplot ///
	...
	keep(*treatment take_up) drop(_cons) xline(0) ///
		...
		swapnames /// swaps coeff & equation names after collecting result
		levels(95) ///
		eqrename(`1'1 = `"Total sales (ITT)"' `1'2 = `"Total sales (TOT)"' `2'1 = `"Costs (ITT)"' `2'2 = `"Costs (TOT)"' `3'1 = `"Profit, ihs (ITT)"' `3'2 = `"Profit, ihs (TOT)"'`4'1 = `"Profit, pct (ITT)"' `4'2 = `"Profit, pct (TOT)"' `5'1 = `"Employees (ITT)"' `5'2 = `"Employees (TOT)"' `6'1 = `" Female Employees (ITT)"' `6'2 = `"Female employees (TOT)"') ///
		xtitle("Treatment coefficient", size(medium)) ///  
		leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
		name(ml_`generate'1_cfplot, replace)
...
coefplot ///
	(`3'1, pstyle(p1)) (`3'2, pstyle(p1)), ///
	keep(*treatment take_up) drop(_cons) xline(0) ///
		asequation /// name of model is used
		swapnames /// swaps coeff & equation names after collecting result
		levels(95) ///
		eqrename( `3'1 = `"Profit, ihs (ITT)"' `3'2 = `"Profit, ihs (TOT)"') ///
		xtitle("Treatment coefficient", size(medium)) ///  
		leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
		name(ml_`generate'2_cfplot, replace)
...
gr export ml_`generate'2_cfplot.png, replace
	...
	* apply program to business performance outcomes
rct_regression_business ihs_sales_w99_k1 ihs_costs_w99_k1 ihs_profit_w99_k1 profit_pct ihs_employes_w99_k1 car_empl1_w99_k1, gen(kbusiness_outcomes)

}		
	
***********************************************************************
* 	PART 13: Midline results - regression table export outcomes
***********************************************************************
{
...
***********************************************************************
* 	PART 14: Midline results - profit sensitivity
***********************************************************************
...
gr export ml_`generate'_cfplot.png, replace				
	...
	* apply program to profit outcomes
...
***********************************************************************
* 	PART 15: Midline results - sales sensitivity
***********************************************************************
* try to have two different columns for coefplot of profits?
{
capture program drop rct_regression_sales // enables re-running
program rct_regression_sales
	version 15							// define Stata version 15 used
	syntax varlist(min=1 numeric), GENerate(string)
		foreach var in `varlist' {		// do following for all variables in varlist seperately	
	* ITT: ancova plus stratification dummies
...
rct_regression_sales ihs_sales_w99_k1 ihs_sales_w99_k2 ihs_sales_w99_k3 ihs_sales_w99_k4 ihs_sales_w99_k5, gen(sales)
...
***********************************************************************
* 	PART 16: Midline results - regression cost outcome
***********************************************************************
{
capture program drop rct_regression_cost // enables re-running
program rct_regression_cost
	version 15							// define Stata version 15 used
	syntax varlist(min=1 numeric), GENerate(string)
		foreach var in `varlist' {		// do following for all variables in varlist seperately	
...
*		tokenize `varlist'
		local regressions `1'1 `2'1 `3'1 // adjust manually to number of variables 
		esttab `regressions' using "rt_`generate'.tex", replace ///
				prehead("\begin{table}[!h] \centering \\ \caption{Cost} \\ \begin{adjustbox}{width=\columnwidth,center} \\ \begin{tabular}{l*{6}{c}} \hline\hline") ///
				posthead("\hline \\ \multicolumn{5}{c}{\textbf{Panel A: Intention-to-treat (ITT)}} \\\\[-1ex]") ///
				fragment ///
				cells(b(star fmt(3)) se(par fmt(3)) p(fmt(3)) rw ci(fmt(2))) ///
				mlabels(, depvars) /// use dep vars labels as model title
		...
		esttab `regressions' using "rt_`generate'.tex", append ///
			...
			* coefplot
coefplot (`1'1, pstyle(p1)) (`1'2, pstyle(p1)) (`2'1, pstyle(p3)) (`2'2, pstyle(p3)), ///
	keep(*treatment take_up) drop(_cons) xline(0) ///
	asequation /// name of model is used
	swapnames /// swaps coeff & equation names after collecting result
	levels(95) ///
	xtitle("Treatment coefficient", size(medium)) ///
	eqrename(`1'1 = `"Cost (ITT)"' `1'2 = `"Cost (TOT)"' `2'1 = `"Turnover (ITT)"' `2'2 = `"Turnover (TOT)"') ///
	leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
	name(ml_`generate'1_cfplot, replace)
	
gr export ml_`generate'1_cfplot.png, replace

coefplot (`3'1, pstyle(p2)) (`3'2, pstyle(p2)), ///
keep(*treatment take_up) drop(_cons) xline(0) ///
	asequation /// name of model is used
	swapnames /// swaps coeff & equation names after collecting result
	levels(95) ///
	xtitle("Treatment coefficient", size(medium)) ///
	eqrename(`3'1 = `"Profit (ITT)"' `3'2 = `"Profit (TOT)"') ///
	leg(off) xsize(4.5) /// xsize controls aspect ratio, makes graph wider & reduces its height
	name(ml_`generate'2_cfplot, replace)
...
gr export ml_`generate'2_cfplot.png, replace
	...
	* apply program to business performance outcomes
rct_regression_cost ihs_costs_w99_k1 ihs_ca_w99_k1 ihs_profit_w99_k1, gen(cost)
